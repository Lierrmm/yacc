...
namespace components
{
		...
		command::add("menu_export", "<menu name> <[optional] subdir>", "exports the specified menu to root\\menu_export\\menus\\subdir\\", [this](command::params params)
			{
				if (params.length() < 2)
				{
					game::native::Com_PrintMessage(0, "^1No menu name given.^7\n", 0);
					return;
				}
				...
				for (int i = 0; i < game::native::ui_context->menuCount; i++)
				{
					if (strcmp(game::native::ui_context->Menus[i]->window.name, params[1]) == 0)
					{
						export_menu(game::native::ui_context->Menus[i], params.length() == 2 ? "" : params[2]);
						return;
					}
				}
				...
				if (const auto& cl_ingame = game::native::Dvar_FindVar("cl_ingame"); cl_ingame)
				{
					if (cl_ingame->current.enabled && game::native::ui_cg_dc)
					{
						for (int i = 0; i < game::native::ui_cg_dc->menuCount; i++)
						{
							if (strcmp(game::native::ui_cg_dc->Menus[i]->window.name, params[1]) == 0)
							{
								export_menu(game::native::ui_cg_dc->Menus[i], params.length() == 2 ? "" : params[2]);
								return;
							}
						}
					}
				...
				game::native::Com_PrintMessage(0, utils::string::va("^1WARNING: ^7Menu %s was not found.\n", params[1]), 0);
			});
		...
		command::add("menu_export_itemdefs", "<menu name> <[optional] subdir>", "exports all itemdefs of the specified menu to root\\menu_export\\itemdefs\\subdir\\", [this](command::params params)
			{
				...
				for (int i = 0; i < game::native::ui_context->menuCount; i++)
				{
					if (strcmp(game::native::ui_context->Menus[i]->window.name, params[1]) == 0)
					{
						export_menu_itemdefs(game::native::ui_context->Menus[i], params.length() == 2 ? "" : params[2]);
						return;
					}
				}
				...
				if (const auto& cl_ingame = game::native::Dvar_FindVar("cl_ingame"); cl_ingame)
				{
					if (cl_ingame->current.enabled && game::native::ui_cg_dc)
					{
						for (int i = 0; i < game::native::ui_cg_dc->menuCount; i++)
						{
							if (strcmp(game::native::ui_cg_dc->Menus[i]->window.name, params[1]) == 0)
							{
								export_menu_itemdefs(game::native::ui_cg_dc->Menus[i], params.length() == 2 ? "" : params[2]);
								return;
							}
						}
					}
	...
	void menu_export::export_menu(game::native::menuDef_t* menu, std::string subdir)
	{
		...
		game::native::Com_PrintMessage(0, "Exporting menu ...\n", 0);
		...
		game::native::Com_PrintMessage(0, utils::string::va("Menu exported to: %s\n", writeFilePath.c_str()), 0);
	}

	//Export all itemdef from a menudef to a specified folder and file name
	void menu_export::export_menu_itemdefs(game::native::menuDef_t* menu, std::string subdir)
	{
		...
		game::native::Com_PrintMessage(0, "Exporting itemDefs ...\n", 0);
		...
		game::native::Com_PrintMessage(0, utils::string::va("ItemDefs exported to: %s\n", writeFilePath.c_str()), 0);
	}
	...
	bool menu_export::set_path(std::string subdir, std::string name)
	{
		...
		const auto& base_path = game::native::Dvar_FindVar("fs_basepath");
		...
		std::string file_path = base_path->current.string + "\\yacc\\menu_export\\"s + subdir;
		file_path += utils::string::ends_with(subdir, "\\"s) ? "" : "\\";
	...
	void menu_export::write_menu_def(game::native::menuDef_t* menuDef)
	{
		tabbing = MENU_TABBING;
	...
	void menu_export::write_item_def(game::native::itemDef_s& itemDef)
	{
		//itemdef open
		menu_file_ << "\n" << tabbing << "itemDef\n";
		menu_file_ << tabbing << "{\n";
	...
	//Write rectangle
	void menu_export::write_rect(game::native::rectDef_s rect)
	{
	...
	void menu_export::write_visible(game::native::statement_s stmt)
	{
	...
	//Write statement_s
	void menu_export::write_statement(std::string name, game::native::statement_s stmt)
	{
		if (stmt.numEntries == 0)
		{
			return;
...
REGISTER_MODULE(components::menu_export)