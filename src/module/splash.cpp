...
namespace splash
{
	...
	public:
		...
		void post_unpack() override
		{
			...
			utils::hook::nop(SELECT_VALUE(0x0, 0x571D1C), 5);
			utils::hook(SELECT_VALUE(0x0, 0x4F80FD), destroy_stub, HOOK_JUMP).install()->quick();
			//utils::hook(SELECT_VALUE(0x0, 0x5D4947), destroy_stub, HOOK_JUMP).install()->quick();
		}
	...
	private:
		...
		{
			module_loader::get<component>()->destroy();
		}
		...
		void destroy() const
		{
			if (this->window_ && IsWindow(this->window_))
			{
				...
				UnregisterClassA("YACC Splash Screen", utils::nt::library{});
			}
		}
		...
		void show()
		{
			...
			wnd_class.lpszClassName = "YACC Splash Screen";
			...
			if (RegisterClassA(&wnd_class))
			{
				...
				if (image_)
				{
					this->window_ = CreateWindowExA(WS_EX_APPWINDOW, "YACC Splash Screen", "YACC",
					                                WS_POPUP | WS_SYSMENU,
...
//REGISTER_MODULE(splash::component)
